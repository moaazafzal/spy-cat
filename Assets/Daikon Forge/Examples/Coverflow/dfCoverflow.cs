...
public class dfCoverflow : MonoBehaviour
{
    ...
    private void layoutSlidesForEditor()
    {
        ...
        var left = 0;
        var top = (container.Height - itemSizeHieght) * 0.5f;
        //				var size = Vector2.one * itemSize;
        var size = new Vector2(itemSize, itemSizeHieght);
            ...
            currentX = new dfAnimatedFloat(start, calculateTargetPosition(), time)
{
    EasingType = dfEasingType.SineEaseOut
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            control.Size = new Vector2(itemSize, itemSizeHieght);
            ...
            if (Application.isPlaying)
            {
                ...
                //Change the scale
                var scale = calcScale(itemPosition.x) * 1.1f;
