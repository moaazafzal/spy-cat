...
public class MainTool : EditorWindow {
	...
	void Update() {
        ...
        if (old_sel_bird_size != select_bird_size)
        {
            ...
            ParticleSystem tmp = birds.GetComponent<ParticleSystem>();

            if (tmp != null)
            {
                var main = tmp.main;
                main.startSize = select_bird_size;
            }
        }
        ...
        if (old_sel_bird_dir != select_bird_dir)
        {
            ...
            {
                var main = tmp.main;
                var velocity = tmp.velocityOverLifetime;

                switch (select_bird_dir)
                {
                    case 0:
                        // Check and invert the X velocity if needed
                        if (velocity.x.constant < 0)
                        {
                            velocity.x = new ParticleSystem.MinMaxCurve(velocity.x.constant * -1.0f);
                            birds.GetComponent<ParticleSystemRenderer>().GetComponent<Renderer>().material.mainTextureOffset = new Vector2(0.0f, 0.5f);
                        }
                        break;
                    ...
                    case 1:
                        ...
                        {
                            velocity.x = new ParticleSystem.MinMaxCurve(velocity.x.constant * -1.0f);
                            birds.GetComponent<ParticleSystemRenderer>().GetComponent<Renderer>().material.mainTextureOffset = new Vector2(0.0f, 0.0f);
                        }
                        break;
        ...
        if (old_sel_bird_speed != select_bird_speed)
        {
            ...
            if (tmp != null)
            {
                ...
                // Determine new velocity based on the direction
                if (velocity.x.constant > 0)
                {
                    velocity.x = new ParticleSystem.MinMaxCurve(0.1f * select_bird_speed);
                    velocity.y = new ParticleSystem.MinMaxCurve(0.1f * select_bird_speed);
                }
                else
                {
                    velocity.x = new ParticleSystem.MinMaxCurve(-0.1f * select_bird_speed);
        ...
        if (old_sel_but_count != select_but_count)
        {
            ...
            ParticleSystem tmp = butterfly.GetComponent<ParticleSystem>();

            if (tmp != null)
            {
                var emission = tmp.emission;
                emission.rateOverTime = select_but_count;
            }
        }
        ...
        if (old_sel_but_dir != select_but_dir)
        {
            ...
            {
                var main = tmp.main;
                var force = tmp.forceOverLifetime;
                var rotation = tmp.rotationOverLifetime;

                switch (select_but_dir)
                {
                    case 0:
                        // Update velocity and texture offset for direction 0
                        if (force.x.constant < 0)
                        {
                            force.x = new ParticleSystem.MinMaxCurve(force.x.constant * -1.0f);
                            force.y = new ParticleSystem.MinMaxCurve(force.y.constant * -1.0f);
                            rotation.z = new ParticleSystem.MinMaxCurve(rotation.z.constant * -1.0f);
                            butterfly.GetComponent<ParticleSystemRenderer>().GetComponent<Renderer>().material.mainTextureOffset = new Vector2(0.0f, 0.5f);
                        }
                        break;

                    case 1:
                        // Update velocity and texture offset for direction 1
                        if (force.x.constant > 0)
                        {
                            force.x = new ParticleSystem.MinMaxCurve(force.x.constant * -1.0f);
                            force.y = new ParticleSystem.MinMaxCurve(force.y.constant * -1.0f);
                            rotation.z = new ParticleSystem.MinMaxCurve(rotation.z.constant * -1.0f);
                            butterfly.GetComponent<ParticleSystemRenderer>().GetComponent<Renderer>().material.mainTextureOffset = new Vector2(0.0f, 0.0f);
                        }
                        break;
        ...
        if (old_sel_fly_dir != select_fly_dir)
        {
            ...
            ParticleSystem tmp = flies.GetComponent<ParticleSystem>();
            ...
            if (tmp != null)
            {
                ...
                {
                    case 0:
                        // Update force and rotation for direction 0
                        if (force.x.constant < 0)
                        {
                            force.x = new ParticleSystem.MinMaxCurve(force.x.constant * -1.0f);
                            force.y = new ParticleSystem.MinMaxCurve(force.y.constant * -1.0f);
                            rotation.z = new ParticleSystem.MinMaxCurve(rotation.z.constant * -1.0f);
                            flies.GetComponent<ParticleSystemRenderer>().GetComponent<Renderer>().material.mainTextureOffset = new Vector2(0.0f, 0.5f);
                        }
                        break;

                    case 1:
                        // Update force and rotation for direction 1
                        if (force.x.constant > 0)
                        {
                            force.x = new ParticleSystem.MinMaxCurve(force.x.constant * -1.0f);
                            force.y = new ParticleSystem.MinMaxCurve(force.y.constant * -1.0f);
                            rotation.z = new ParticleSystem.MinMaxCurve(rotation.z.constant * -1.0f);
                            flies.GetComponent<ParticleSystemRenderer>().GetComponent<Renderer>().material.mainTextureOffset = new Vector2(0.0f, 0.0f);
                        }
                        break;
        ...
        if (old_sel_rain_intensity != select_rain_intensity)
        {
            ...
            ParticleSystem tmp = rain.GetComponent<ParticleSystem>();

            if (tmp != null)
            {
                var main = tmp.main;
                main.startSize = select_rain_intensity * 0.01f;
            }
        }
        ...
        if (old_sel_snow_intensity != select_snow_intensity)
        {
            ...
            ParticleSystem tmp = snow.GetComponent<ParticleSystem>();

            if (tmp != null)
            {
                var emission = tmp.emission;
                emission.rateOverTime = select_snow_intensity * 10.0f;
            }
        }
